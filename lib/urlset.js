...
provider.prototype.load = function(files) {
	...
	if (_.isArray(files)) {
		_.forEach(files, function(f) {
			return self.loadData(internal.readFile(f));
		});
	}
...
provider.prototype.loadData = function(data) {
	if (_.isString(data)) {
		data = internal.formatData(data, this._options.formater);
	}
	if (_.isObject(data)) {
...
provider.prototype.add = function(url, name, path) {
	...
	function fn() {
		...
		var aparams = url.args < alist.length && _.isObject(alist[alist.length - 1]) ? alist[alist.length - 1] : {};
		var aurl = url.url;
		...
		aparams = _.clone(aparams);

		for (var prop in aparams) {
		  var value = aparams[prop];
		  if (aurl.indexOf('${' + prop + '}') > -1) {
		    delete aparams[prop];
		  }
		  aurl = aurl.replace(new RegExp("\\$\\{" + prop + "\\}", "g"), value);
		}
		return internal.normalizeUrl(self, aurl, url, aparams);
	}
...
provider.prototype.buildUrls = function() {
	var self = this;
	internal.buildUrls(this, this._data);
	return this;
};
...
internal.buildUrls = function(self, data, parentName, path, names) {
	path = path || [];
	names = names || [];
	...
	objectProperties.forEach(function(name) {
		var _names = _.clone(names),
			_path = _.clone(path);
		names.push(name);
		internal.buildUrls(self, data[name], name, path, names);
		if (self._options.debug)
...
internal.normalizeUrl = function(self, url, urlData, params) {
	var usedParams = [];
	//fill extra params:
		...
		for (var name in params) {
			...
			if (param) {
				if (param.useDefault === true) {
					...
					url = internal.formatSPUrl(url, param, value);
				} else if (param.value !== value) {
					url = internal.formatSPUrl(url, param, value);
				}
			} else { //set as query param:
				url = internal.setUrlQParam(url, name, value);
			}
		}
	}
	...
	if (sparams.length > 0) {
		_.forEach(sparams, function(param) {
			if (!_.contains(usedParams, param.name) && param.useDefault === true && !(_.isUndefined(param.value) || _.isNull(param.value))) {
				url = internal.formatSPUrl(url, param, param.value);
			}
		});
	}
...
internal.formatSPUrl = function(url, param, value) {
	if (value === null || value === undefined) return url;
	if (param.format === 's') {
		url = '/' + value + url;
	} else {
		url = internal.setUrlQParam(url, param.name, value);
	}
	return url;
};

internal.setUrlQParam = function(url, name, value) {
	if (value === null || value === undefined) return url;
	var prefix = _.contains(url, '?') ? '&' : '?';
	return url + prefix + name + '=' + encodeURIComponent(value);
};

internal.formatData = function(data, formater) {
